...
class LogoDataset(data.Dataset):
    CLASSES = ['Adidas', 'Apple', 'BMW', 'Citroen', 'Cocacola', 'DHL', 'Fedex', 'Ferrari', 
               'Ford', 'Google', 'Heineken', 'HP', 'Intel', 'McDonalds', 'Mini', 'Nbc', 'Nike', 
               'Pepsi', 'Porsche', 'Puma', 'RedBull', 'Sprite', 'Starbucks', 'Texaco', 'Unicef', 
               'Vodafone', 'Yahoo']
        
    def __init__(self, root, transform=None, target_transform=None, dataset_name='Flickr Logos 27', label="flickr_logos_27"):
        filename = 'flickr_logos_27_dataset_training_set_annotation.txt'
        self.root = root
        csv_data = pd.read_csv(self.root + "/" + filename, sep=' ', 
                        header=None, usecols=[0, 1, 2, 3, 4, 5, 6], 
                        names=['filename', 'brand', 'train_subset', 'xmin', 'ymin', 'xmax', 'ymax'])
        self.classes = LogoDataset.CLASSES
    ...
    def __getitem__(self, index):
        ...
        sample = cv2.imread(self.root + "/images/" + im_name)
